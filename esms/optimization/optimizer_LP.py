...
class EnergyOptimizerLP:
    """
    Day-ahead energy optimizer using LP.
    ...
    def __init__(
        ...
        self.export_price_forecast = np.array(export_price_forecast) if export_price_forecast is not None else np.zeros_like(price_forecast)
    ...
    def _validate_inputs(self):
        ...
        if len(self.export_price_forecast) != n_timesteps:
            raise ValueError("export_price_forecast must have same length as load_forecast")
    ...
    def build_model(self) -> ConcreteModel:
        ...
        model.Load = Param(model.T, initialize={t: self.load_forecast[t] for t in model.T})
        model.Price = Param(model.T, initialize={t: self.price_forecast[t] for t in model.T})
        model.ExportPrice = Param(model.T, initialize={t: self.export_price_forecast[t] for t in model.T})
        ...
        # Decision Variables
        model.charge = Var(model.B, model.T, domain=Rea, doc="Battery charge power (kW)")
        model.discharge = Var(model.B, model.T, domain=NonNegativeReals, doc="Battery discharge power (kW)")
        model.soc = Var(model.B, model.T, domain=NonNegativeReals, doc="State of charge (kWh)")
        model.grid_import = Var(model.T, domain=NonNegativeReals, doc="Grid import power (kW)")
        model.grid_export = Var(model.T, domain=NonNegativeReals, doc="Grid export power (kW)")
        
        # Binary variable for charge/discharge state (MILP constraint)
        model.u = Var(model.B, model.T, domain=Binary, doc="Charge state binary (1=can charge, 0=can discharge)")
        
        # Objective: Minimize total cost
        def objective_rule(model):
            return sum(
                (model.grid_import[t] * model.Price[t] - model.grid_export[t] * model.ExportPrice[t]) * model.dt
                for t in model.T
            )
        ...
        def energy_balance_rule(model, t):
            ...
            return (
                model.Load[t] ==
                model.PV[t] + total_discharge + model.grid_import[t] - total_charge - model.grid_export[t]
            )
        ...
        self.model = model
        logger.info(f"Model built with {len(model.T)} timesteps and {len(model.B)} batteries")
    ...
    def solve(self, verbose: bool = False) -> Dict[str, Any]:
        ...
        if self.results.solver.status == SolverStatus.ok:
            ...
            elif self.results.solver.termination_condition == TerminationCondition.feasible:
                logger.warning("Feasible solution found (not proven optimal)")
                return self._extract_results()
        
        logger.error(f"Solver failed: {self.results.solver.status}")
        logger.error(f"Termination condition: {self.results.solver.termination_condition}")
