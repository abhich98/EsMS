...
def main():
    parser = argparse.ArgumentParser(
        description="Simulate Real-Time prices from Day-Ahead prices."
    )
    parser.add_argument(
        "--data_file",
        type=str,
        required=True,
        help="Path to the Day-Ahead prices Excel file",
    )
    parser.add_argument(
        "--noise_params_file",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to save the simulated Real-Time prices CSV file",
    )
    ...
    da_prices_df.rename(
        columns={"Date": "timestamp", "Energy price (EUR/kWh)": "price"}, inplace=True
    )
    ...
    rt_prices_df.rename(
        columns={"timestamp": "Date", "rt_price": "RT energy price (EUR/kWh)"},
        inplace=True,
    )
    rt_prices_df.sort_values("Date", inplace=True)

    logging.info("Saving simulated Real-Time prices to %s", args.output_file)
    rt_prices_df.to_csv(args.output_file, index=False)
