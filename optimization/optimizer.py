...
class EnergyOptimizer:
    ...
    def _validate_inputs(self):
        ...
        if len(self.load_forecast) != n_timesteps:
            raise ValueError("load_forecast must have same length as pv_forecast")
        
        if len(self.price_forecast) != n_timesteps:
            raise ValueError("price_forecast must have same length as pv_forecast")
    ...
    def build_model(self) -> ConcreteModel:
        ...
        # Decision Variables
        model.charge = Var(model.B, model.T, domain=NonNegativeReals, doc="Battery charge power (kW)")
        ...
        def objective_rule(model):
            return sum(
                (model.grid_import[t] * model.Price[t] - model.grid_export[t] * model.Price[t]) * model.dt
                for t in model.T
            )
    ...
    def solve(self, tee: bool = False) -> Dict[str, Any]:
        """
    ...
    def _extract_results(self) -> Dict[str, Any]:
        ...
        model = self.model
        n_timesteps = len(model.T)
        n_batteries = len(model.B)
