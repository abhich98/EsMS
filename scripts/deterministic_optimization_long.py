...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run deterministic optimization for a selected period of time."
    )

    parser.add_argument("--data_file", type=str, help="Path to the dataset Excel file")
    parser.add_argument(
        "--battery_file", type=str, help="Path to the battery configuration JSON file"
    )
    parser.add_argument(
    ...
    pv_forecast, load_forecast, price_forecast = (
        forecast_df["PV generation (kW)"].values,
        forecast_df["Consumption (kW)"].values,
        forecast_df["Energy price (EUR/kWh)"].values,
    )
    ...
    logger.info(
        f"Price range: {price_forecast.min():.3f} - {price_forecast.max():.3f} EUR/kWh"
    )
    ...
    try:
        results = optimizer.solve(
            solver_name=solver_to_use, verbose=True, **solver_args
        )
        ...
        logger.info(
            f"Potential Total Cost without PV and management: {sum(l * p * timestep_hours for l, p in zip(load_forecast, price_forecast)):.2f} EUR"
        )
        ...
        for bat_result in results["batteries"]:
            ...
            logger.info(
                f"    Total charge: {total_charge * optimizer.timestep_hours:.1f} kWh"
            )
            logger.info(
                f"    Total discharge: {total_discharge * optimizer.timestep_hours:.1f} kWh"
            )
        ...
        logger.info(
            f"   Total import: {total_import * optimizer.timestep_hours:.1f} kWh"
        )
        logger.info(
            f"   Total export: {total_export * optimizer.timestep_hours:.1f} kWh"
        )
